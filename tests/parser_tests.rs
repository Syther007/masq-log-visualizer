use masq_log_visualizer::parser::{get_table_data, scan_directory};
...
fn test_database_extraction() {
    ...
    for (node_name, node) in &nodes {
        if !node.database.tables.is_empty() {
            println!(
                "Node {} has {} database tables",
                node_name,
                node.database.tables.len()
            );
            ...
            for (table_name, table_data) in &node.database.tables {
                ...
                assert_eq!(
                    table_data.rows.len(),
                    0,
                    "Rows should be empty (on-demand loading)"
                );
            }
...
fn test_serialization_deserialization() {
    use masq_log_visualizer::models::{DatabaseData, GossipEntry, NeighborhoodEdge, NodeData};
    use std::collections::HashMap;

    let node = NodeData {
        name: "test_node".to_string(),
        neighborhood: vec![NeighborhoodEdge {
...
fn test_scan_empty_directory() {
    ...
    if let Ok(nodes) = result {
        assert!(
            nodes.is_empty(),
            "Empty directory should result in no nodes"
        );
    }
