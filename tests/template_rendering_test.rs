...
async fn test_flat_structure_template_rendering() {
    ...
    let app = Router::new()
        .route("/node/:node_name", get(masq_log_visualizer::routes::node_view))
    ...
    assert_eq!(response.status(), StatusCode::OK, "Should render page successfully");
    ...
    assert!(body_str.contains("const currentLogFile ="), 
        "HTML should contain currentLogFile variable declaration");
    
    assert!(body_str.contains("1-MASQNode_rCURRENT.log"),
    ...
    let fetch_lines: Vec<&str> = body_str.lines()
        .filter(|line| line.contains("/api/logs/") && line.contains("/range"))
        .collect();
    
    for line in &fetch_lines {
        assert!(line.contains("${currentLogFile}") || line.contains("${ currentLogFile}"),
            "API fetch should use currentLogFile variable, found: {}", line);
    }
