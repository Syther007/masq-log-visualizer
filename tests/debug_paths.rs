...
fn test_current_log_file_extraction() {
    ...
    if let Some(node) = nodes.get("1") {
        ...
        assert!(current_log_file.starts_with("1-"), 
            "Expected filename to start with '1-', got: {}", current_log_file);
        assert!(current_log_file.contains("MASQNode_rCURRENT.log"),
            "Expected filename to contain 'MASQNode_rCURRENT.log', got: {}", current_log_file);
    } else {
    ...
    if test_dir2.exists() {
        ...
        if let Some((_, node)) = nodes2.iter().next() {
            ...
            assert!(!current_log_file.starts_with(char::is_numeric),
                "Nested structure filename should not start with number: {}", current_log_file);
        }
    }
}
