...
async fn setup_test_app() -> Option<(axum::Router, AllNodesData)> {
    ...
    eprintln!("Loaded templates: {:?}", tera.get_template_names().collect::<Vec<_>>());
    ...
    let app = axum::Router::new()
        ...
        .route("/api/logs/:node_name/:file_name/range", get(masq_log_visualizer::routes::get_log_range))
        .route("/api/logs/:node_name/:file_name", get(masq_log_visualizer::routes::download_log))
    ...
    for (node_name, node_data) in &nodes_data {
        if !node_data.log_files.is_empty() {
            ...
            let response = app
                .oneshot(
                    Request::builder()
                        .uri(&format!("/api/logs/{}/{}/range?fromEnd=true&lines=10", node_name, log_file))
                        .body(Body::empty())
                        .unwrap()
                )
                .await
